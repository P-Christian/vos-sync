...
export async function POST(req: NextRequest) {
  try {
    ...
    if (companyId && uploadedFile) {
      ...
      // Transactional Replacement: If an existing document of the same document_type exists for this company, remove old DB record and file asset (except for OTHER_DOCUMENT which allows multiple files)
      const isMultiDocType = String(documentType).toUpperCase() === "OTHER_DOCUMENT" || String(documentType).toUpperCase() === "OTHER";
      if (!isMultiDocType) {
        try {
