...
export const metadata = {
  title: "Review Candidate | VOS Sync client Portal",
  description: "Review applicant details for your job postings on VOS Sync.",
};
...
export default async function ApplicantDetailPage({ params }: PageProps) {
  ...
  const appId = Number(id) || undefined;
  ...
  return (
    <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
      ...
      <main className="min-h-0 min-w-0 flex-1 overflow-y-auto overflow-x-hidden p-4 sm:p-8 bg-secondary/10">
        <ApplicantsModule initialApplicationId={appId} />
      </main>
    </div>
  );
}
