// src/modules/school-admin/school-management/hooks/useSchools.ts
"use client";
...
export function useSchools() {
  ...
  const fetchSchools = useCallback(async (status?: string, search?: string) => {
    ...
    try {
      ...
      const res = await fetch(`/api/school-admin/schools${q}`);
  ...
  const createSchool = useCallback(async (formData: unknown) => {
    ...
    setSuccessMessage("");
    try {
      const res = await fetch("/api/school-admin/schools", {
        method: "POST",
