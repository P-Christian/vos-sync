// src/modules/vos-admin/school-management/hooks/useSchoolDetail.ts
"use client";
...
export function useSchoolDetail() {
  ...
  const fetchSchoolDetail = useCallback(async (schoolId: number) => {
    ...
    try {
      ...
      const coursesRes = await fetch(`/api/vos-admin/schools/${schoolId}/courses`);
  ...
  const addCourse = useCallback(async (schoolId: number, formData: unknown) => {
    ...
    setSuccessMessage("");
    try {
      const res = await fetch(`/api/vos-admin/schools/${schoolId}/courses`, {
        method: "POST",
  ...
  const updateCourseStatus = useCallback(async (schoolId: number, courseId: number, currentStatus: string) => {
    ...
    try {
      ...
      // But we can patch the course list API conceptually, or just patch /api/vos-admin/school-courses/[id]
      ...
      const res = await fetch(`/api/vos-admin/schools/${schoolId}/courses/${courseId}`, {
         method: "PATCH",
         headers: { "Content-Type": "application/json" },
